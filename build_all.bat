...
set ret=0
set def_proj=EC200UEU_AA
set buildtype_src=new
::%1
set buildoption=
if "%buildtype_src%"=="" (
	set buildoption=r
...
@REM if /i "%3" == "" (
@REM 	echo=
@REM 	echo we need your version label...
@REM 	echo=
@REM 	goto helpinfo
@REM )
...
echo.
	echo ********* Customer  copy .\hex %version_path%\prepack\
...
echo.
	echo ********* Customer  copy .\target.cmake %version_path%\prepack\

xcopy .\target.cmake %version_path%\prepack\
echo.
	echo ********* Customer  copy .\hex\examples\*.* %version_path%\app\
xcopy /y .\hex\*.* %version_path%\app\
...
: Customer build FOTA file
echo.
	echo.
	echo ********************        Create FOTA File        ***********************
	echo ********************START COPY FILE %BUILD_TARGET%_%BUILD_RELEASE_TYPE%\hex\%buildver%.pac***********************
	echo ****************************************************************
xcopy /y out\%BUILD_TARGET%_%BUILD_RELEASE_TYPE%\hex\%buildver%.pac tools\win32\
cd tools\win32
dtools fotacreate2 --single-pac %buildver%.pac,setting\fota8910.xml %buildver%_FOTA.pac -d v 
cd /d %PROJECT_ROOT%
